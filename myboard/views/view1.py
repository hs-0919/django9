...
def listFunc(request):
    # data_all = BoardTab.objects.all().order_by('-id')  # 아직 댓글 코드 없다
...
def insertOkFunc(request):
    if request.method == 'POST':
        try:
            gbun = 1  # 그룹넘버 group number 구하기
            datas = BoardTab.objects.all()
            if datas.count() != 0:
                gbun = BoardTab.objects.latest('id').id + 1  # 마지막 아이디 자료에서 플라스 1해서 읽어옴
